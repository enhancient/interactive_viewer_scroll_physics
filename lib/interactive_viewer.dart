...
import 'package:flutter/foundation.dart' show Float64List, clampDouble;
...
class InteractiveViewer extends StatefulWidget {
  ...
  ///
  /// The [snapScaleToFit] parameter controls whether, when ending a scale gesture,
  /// the widget will snap to at least the larger of [minScale] or the fit-to-content
  /// scale (the scale at which the content fits entirely in the viewport). If false,
  /// only [minScale] is used as the lower bound.
  InteractiveViewer({
    required this.child,
    super.key,
...
class _InteractiveViewerState extends State<InteractiveViewer> with TickerProviderStateMixin {
  ...
  double _lastScale = 1.0; // track incremental pinch scale factor per frame
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    /// Handle translations out of boundaries
    ...
    if (widget.scrollPhysics != null && _gestureType == _GestureType.pan) {
      ...
      final Offset currentOffset = _getMatrixTranslation(_transformationController!.value) * -1;
  ...
  Matrix4 _matrixScale(Matrix4 matrix, double scale) {
    ...
    final double rawDesiredScale = currentScale * scale;

    // Early return if not allowed to zoom outside bounds
    if (!_shouldAllowScale(rawDesiredScale)) {
      final double clampedTotalScale = clampDouble(rawDesiredScale, widget.minScale, widget.maxScale);
    ...
    if (widget.scrollPhysics != null &&
        ...
        ((rawDesiredScale < widget.minScale) || (rawDesiredScale > widget.maxScale))) {
      // Compute current and desired absolute scale
      final double contentWidth = _boundaryRect.width * currentScale;
      final double desiredContentWidth = _boundaryRect.width * rawDesiredScale;
      final double contentHeight = _boundaryRect.height * currentScale;
      final double desiredContentHeight = _boundaryRect.height * rawDesiredScale;
      ...
      // Give physics half the delta to mimic one-sided boundary pull
      final double halfDeltaX = deltaX / 2.0;
      final double halfDeltaY = deltaY / 2.0;
      final double adjustedHalfX = widget.scrollPhysics!.applyPhysicsToUserOffset(
        metricsX,
        halfDeltaX// * rawDesiredScale,
      );
      final double adjustedHalfY = widget.scrollPhysics!.applyPhysicsToUserOffset(
        metricsY,
        halfDeltaY// * rawDesiredScale,
      );
      final double adjustedX = adjustedHalfX * 2.0;// / rawDesiredScale;
      final double adjustedY = adjustedHalfY * 2.0;// / rawDesiredScale;
      final double adjustedXpercent = adjustedX / deltaX;
      ...
      print(
        'currentScale: $currentScale newScaleX: $newScaleX, (adjustedX: $adjustedX [${adjustedXpercent.toStringAsFixed(3)}] newScaleY: $newScaleY, factor: $factor',
      );
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType!) {
      case _GestureType.scale:
        /*
        // Incremental scale: apply only the change since last update
        final double incremental = details.scale / _lastScale;
        _lastScale = details.scale;
        _transformationController!.value = _matrixScale(_transformationController!.value, incremental); */
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    switch (_gestureType) {
      case _GestureType.pan:
        ...
        if (_transformationController!.value.getMaxScaleOnAxis() > widget.maxScale) {
          _currentAxis = null;
          return;
        }
        ...
        if (widget.scrollPhysics != null) {
          ...
          final Vector3 currentTranslation = _transformationController!.value.getTranslation();
      ...
      case _GestureType.scale:
        ...
        _snapStartMatrix = _transformationController!.value.clone();
        ...
        if (widget.snapScaleToFit) {
          ...
          lowerBound = widget.minScale > fitScale ? widget.minScale : fitScale;
        } else {
        ...
        if (endScale != targetScale) {
          ...
          final double scaleFactor = targetScale / endScale;
          // Compute the center of the viewport in scene coordinates
          final Size viewSize = _viewport.size;
          final Offset centerLocal = Offset(viewSize.width / 2, viewSize.height / 2);
          final Offset centerScene = _transformationController!.toScene(centerLocal);
          // Build a delta matrix that scales around the viewport center
          final Matrix4 delta = Matrix4.identity()
            ..translate(centerScene.dx, centerScene.dy)
            ..scale(scaleFactor)
            ..translate(-centerScene.dx, -centerScene.dy);
          // Apply the delta on top of the current matrix for snapping
          _snapTargetMatrix = _snapStartMatrix.clone()..multiply(delta);
  ...
  void _onAnimate() {
    ...
    if (widget.scrollPhysics != null) {
      ...
      final Offset simulationScene = _transformationController!.toScene(simulationOffset);
  ...
  void initState() {
    ...
    _snapController = AnimationController(vsync: this, duration: const Duration(milliseconds: 150));
  }
